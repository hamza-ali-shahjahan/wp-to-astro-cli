...
const FIXTURE_INPUT = path.resolve(__dirname, "fixtures/input/sample.wxr");
const FIXTURE_EXPECTED = path.resolve(__dirname, "fixtures/expected");
...
async function listTree(root: string): Promise<string[]> {
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const e of ents) {
      ...
      if (e.isDirectory()) {
        // skip .git side-effect; tested separately
...
describe("golden migration: WXR fixture → Astro project", () => {
  let outDir: string;

  beforeAll(async () => {
    outDir = await fs.mkdtemp(path.join(os.tmpdir(), "wp-to-astro-golden-"));
  ...
  it("emits byte-identical file contents", async () => {
    ...
    for (const rel of files) {
      ...
      const expectedContent = await fs.readFile(path.join(FIXTURE_EXPECTED, rel), "utf-8");
